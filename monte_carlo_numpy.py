...
def calculate_total_pressure(coordinates, box_length, cutoff, reduced_temperature):
    ...
    vir = calculate_total_virial(coordinates=coordinates, box_length=box_length, cutoff=cutoff)
    volume = box_length ** 3
...
def generate_random_coordinates_unused(box_length, density):
    """
    Generate random coordinates in a box.
    ...
    num_atoms = round(density * box_length ** 3)
    print(f'Placing {num_atoms} particles.')
...
def run_simulation(coordinates, box_length, cutoff, reduced_temperature, num_steps,max_displacement=0.1, freq=1000):
    ...
    for step in range(num_steps):
        ...
        if step % freq == 0:
            pressure = calculate_total_pressure(coordinates, box_length, cutoff, reduced_temperature)
